...
import { apiDeleteChat, apiGetChats } from '../../api/chat';
...
function ChatTab(chatTabProps: ChatTabProps) {
  const { name, image: photo, chatId, messages, selectedChat, onClick, clearChat } = chatTabProps;
  ...
  const eraseChat = async () => {
    await apiDeleteChat(selectedChat || "")
    await apiGetChats()
    clearChat()
  };
  ...
  return (
    <ContextMenu menuComponent={<ChatTabContextMenu />}>
      <Container id="chatTab" isSelected={selectedChat === chatId} onClick={onClick}>
        <Wrapper>
          <ChatPhoto>
            <img src={`${urlApi}/${photo}`} alt="ProfilePhoto" className="image" />
          </ChatPhoto>
          <ChatInfo>
            <ChatInfoWrapper>
              <Name>{name}</Name>
