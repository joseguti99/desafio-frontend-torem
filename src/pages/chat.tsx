...
import { Chat, LogoType, UserDataState } from '../types/chat';
...
import { apiGetChats, apiSendMessage } from '../api/chat';
...
function HomeChat() {
  ...
  const handleSendMsg = async () => {
    if (msgEntry.length) {
      await apiSendMessage({ message: msgEntry, chatId: selectedChat })
      setMsgEntry('');
    } else {
      NotificationFailure('No puedes enviar mensajes vacios')
    }
  };
